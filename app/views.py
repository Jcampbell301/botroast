    ...
    if form.validate_on_submit():
        ...
        if not present:  # Else, create new user in db
            new_user = models.User(
                bot_id=form.bot_id.data, acc_tok=form.tok_id.data, group_id=int(form.group_id.data))
...
def get_analysis():
    ...
    if form.validate_on_submit():
        ...
        MSG = anal.get_all_msg(acc_tok=form.tok_id.data,
                               gid=form.group_id.data)
        results = anal.get_activity(MSG)
        anal.graph(form.group_id.data, results[
        ...
        ret += 'Total Number of Messages: ' + \
            str(df['Message Frequency'].sum()) + '<br>'
        ret += 'Total Number of Words: ' + str(df['Words'].sum()) + '<br>'
        ret += 'Total Likes: ' + str(df['Likes Received'].sum()) + '<br>'
        ret += 'Total Days: ' + str((datetime.datetime.fromtimestamp(
            MSG[0]['created_at']) - datetime.datetime.fromtimestamp(MSG[-1]['created_at'])).days) + '<br>'
        ...
        with open('./app/templates/' + file_name, 'w') as html_file:
            html_file.truncate()
            html_file.write(ret.encode('utf-8'))
        ...
        # return
        # flask.render_template(str(form.group_id.data)+'_ANALYTICS.html',
        # m_url=m_url, act_url=act_url, l_url=l_url, r_url=r_url)
